...
class Utils:
    ...
    @staticmethod
    def matchScores(lgraph, rgraph, left_seeds, \
        ...
        l_mapped_neighbr = Utils.filterUnmappedNeighbrs(lnode_out_neighbrs, lgraph_visit_dict)
        for rnode in rgraph.nodes:
            ...
            r_mapped_neighbr = Utils.filterUnmappedNeighbrs(rnode_out_neighbrs, rgraph_visit_dict)
            # for every 'l_mapped_neighbr' node check if it can be found as a pair with
            # any one of the 'r_mapped_neighbr' in the seeds_node_pairs
            match_count = 0
            for lneighbr in l_mapped_neighbr:
                for rneighbr in r_mapped_neighbr:
    ...
    @staticmethod
    def filterUnmappedNeighbrs(neighbrs, visit_dict):
        filtered_neighbrs = []
        for n in neighbrs:
            if visit_dict[n]:
